...
/**
 * makeBlockTagFunc - make a block function
 * @param {string} tag - tag
 * @param {string} blockname - name for block
 * @param {string} name - name
 * @return {function} func - a function for blockfunc
 */
function makeBlockTagFunc(tag, blockname, name) {
  return (state, startLine, endLine, silent) => {
    // let openDelim;
    // let len;
    let params;
    ...
    if (pos + tag.length > max) {
      return false;
    }

    const tagDelim = state.src.slice(pos, pos + tag.length);
    ...
    state.line = nextLine + 1; // + (haveEndMarker ? 1 : 0);
...
exports.makeBlockTagFunc = makeBlockTagFunc;
